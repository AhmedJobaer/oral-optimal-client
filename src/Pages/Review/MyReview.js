...
const MyReview = () => {
    ...
    useEffect(() => {
        fetch(`https://b6a11-service-review-server-side-ahmed-jobaer.vercel.app/myReview?email=${user.email}`)
            .then(res => res.json())
    ...
    const handleDelete = id => {
        ...
        if (proceed) {
            fetch(`https://b6a11-service-review-server-side-ahmed-jobaer.vercel.app/myReview/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
